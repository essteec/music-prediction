"""
Experiment B: Gated Fusion MLP (Phase 4)

Adds a global gating network on top of Experiment A's per-modality encoders.
The gate learns per-sample importance weights across the 6 modalities,
allowing the model to dynamically attend more to acoustic features for
energy-heavy songs and more to lyrics for emotionally ambiguous ones.

Missing modalities (zero-padded rows) are masked out before softmax so gate
mass is never wasted on absent information.

Hypothesis:
  "Can the model learn *which modalities to trust* on a per-sample basis?"

Expected result vs Experiment A:
  - Valence and Popularity improve (gate learns to down-weight acoustics)
  - Energy / Danceability stay ≥ A (gate learns to up-weight acoustics here)
  - Average R² ≥ 0.48

Usage (from project root):
    python dl/07_gated_multimodal.py
    python dl/07_gated_multimodal.py --epochs 150 --batch_size 512
"""
...
from utils.fusion import GatedFusionMLP
...
@torch.no_grad()
def inspect_gates(model, loader, device, n_batches: int = 20):
    """Print average modality gate weights to understand what the model learns."""
    model.eval()
    modality_names = ['metadata', 'mpnet', 'vggish', 'mert', 'panns', 'mel_stats']
    all_gates = []
    ...
    for i, batch in enumerate(loader):
        ...
        raw_inputs = modalities

        # Re-run encoder + gate logic (replicate GatedFusionMLP internals)
        from utils.fusion import MODALITY_DIMS, ENCODED_DIM, NUM_MODALITIES
        encs = [model.encoders[k](v) for k, v in zip(MODALITY_DIMS.keys(), raw_inputs)]
        presence = torch.stack(
            [(x.abs().sum(-1) > 0).float() for x in raw_inputs], dim=-1
        )
        stacked = torch.stack(encs, dim=1)
        summary = stacked.mean(dim=1)
        logits  = model.gate_net(summary)
        logits  = logits + (1.0 - presence) * (-1e9)
        gates   = torch.softmax(logits, dim=-1)
        all_gates.append(gates.cpu())

    avg_gates = torch.cat(all_gates).mean(dim=0).numpy()
    print("\n  Average learned gate weights (higher = model relies on this modality more):")
    for name, w in zip(modality_names, avg_gates):
        bar = "█" * int(w * 50)
        print(f"    {name:<12} {w:.4f}  {bar}")


def main():
    parser = argparse.ArgumentParser(description="Experiment B: Gated Fusion MLP")
    ...
    exp_name  = f"exp_b_gated_fusion_{timestamp}"

    print("=" * 70)
    print("Experiment B: Gated Fusion MLP")
    print("Phase 4 — Global modality attention / learned trust weights")
    print("=" * 70)
    ...
    # ── Gate inspection ───────────────────────────────────────────────────────
    print("\n" + "=" * 70)
    print("Gate Analysis — What did the model learn to rely on?")
    print("=" * 70)
    inspect_gates(model, val_loader, device)
    ...
    for target, values in test_metrics.items():
        rows.append({'timestamp': timestamp, 'experiment': 'B', 'model': 'GatedFusionMLP',
    ...
    print(f"{'Target':<14} {'Phase1B':>8}  {'ExpB':>8}  {'Delta':>8}  {'ML Base':>8}")
    print("-" * 55)
    for t in TARGET_NAMES:
        r2   = test_metrics[t]['r2']
        diff = r2 - phase1b[t]
        sign = "+" if diff >= 0 else ""
        beat = " ← beats ML!" if r2 > ml_base[t] else ""
        print(f"{t:<14} {phase1b[t]:>8.4f}  {r2:>8.4f}  {sign}{diff:>7.4f}  {ml_base[t]:>8.4f}{beat}")
