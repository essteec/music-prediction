"""
MERT Models - Base Features + MERT Audio Embeddings
Trains models with base features (audio, text, sentiment) + MERT embeddings (768-d).
...
print("=" * 80)
print("MERT MODELS - BASE FEATURES + MERT AUDIO EMBEDDINGS (1566 FEATURES)")
print("=" * 80)
...
models_dir = REPO_ROOT / 'ml' / 'models' / 'saved' / 'mert_test'
...
# MERT Embeddings
try:
    X_train_mert = np.load(features_dir / 'X_train_mert.npy')
    X_val_mert = np.load(features_dir / 'X_val_mert.npy')
    print(f"✓ MERT Embeddings: train={X_train_mert.shape}, val={X_val_mert.shape}")
except FileNotFoundError:
    print("\n❌ ERROR: MERT Embeddings not found!")
    print("Run ml/preprocessing/data_splitting.py first.")
    exit(1)

# Combine ALL features
print("\nCombining all features...")
X_train = np.hstack([X_train_audio, X_train_text, X_train_sentiment, X_train_mpnet, X_train_mert])
X_val = np.hstack([X_val_audio, X_val_text, X_val_sentiment, X_val_mpnet, X_val_mert])

print(f"\n{'=' * 80}")
print(f"COMBINED FEATURE MATRIX - MERT")
print(f"{'=' * 80}")
...
print(f"  - MERT:      768 features")
print(f"  - TOTAL:    1566 features")
print(f"{'=' * 80}")
...
for target in targets:
    ...
    all_results.append({'target': target, 'model': 'Mean', 'features': 'mert', 'rmse': rmse_mean, 'mae': mae_mean, 'r2': r2_mean})
    ...
    all_results.append({'target': target, 'model': 'Linear', 'features': 'mert', 'rmse': rmse_lr, 'mae': mae_lr, 'r2': r2_lr})
    joblib.dump(lr, models_dir / f'linear_mert_{target}.pkl')
    ...
    all_results.append({'target': target, 'model': 'Ridge', 'features': 'mert', 'rmse': rmse_ridge, 'mae': mae_ridge, 'r2': r2_ridge})
    joblib.dump(ridge, models_dir / f'ridge_mert_{target}.pkl')
    ...
    all_results.append({'target': target, 'model': 'XGBoost', 'features': 'mert', 'rmse': rmse_xgb, 'mae': mae_xgb, 'r2': r2_xgb})
    joblib.dump(xgb_model, models_dir / f'xgboost_mert_{target}.pkl')
...
results_path = results_dir / 'mert_test' / f'mert_results_{timestamp}.csv'
