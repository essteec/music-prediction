"""
VGGish Models - Base Features + VGGish Audio Embeddings
Trains models with base features (audio, text, sentiment) + VGGish embeddings (128-d).
...
print("=" * 80)
print("VGGISH MODELS - BASE FEATURES + VGGISH AUDIO EMBEDDINGS (926 FEATURES)")
print("=" * 80)
...
models_dir = REPO_ROOT / 'ml' / 'models' / 'saved' / 'vggish_test'
...
# VGGish Embeddings
try:
    X_train_vggish = np.load(features_dir / 'X_train_vggish.npy')
    X_val_vggish = np.load(features_dir / 'X_val_vggish.npy')
    print(f"✓ VGGish Embeddings: train={X_train_vggish.shape}, val={X_val_vggish.shape}")
except FileNotFoundError:
    print("\n❌ ERROR: VGGish Embeddings not found!")
    print("Run ml/preprocessing/data_splitting.py first.")
    exit(1)

# Combine ALL features
print("\nCombining all features...")
X_train = np.hstack([X_train_audio, X_train_text, X_train_sentiment, X_train_mpnet, X_train_vggish])
X_val = np.hstack([X_val_audio, X_val_text, X_val_sentiment, X_val_mpnet, X_val_vggish])

print(f"\n{'=' * 80}")
print(f"COMBINED FEATURE MATRIX - VGGISH")
print(f"{'=' * 80}")
...
print(f"  - VGGish:    128 features")
print(f"  - TOTAL:    926 features")
print(f"{'=' * 80}")
...
for target in targets:
    ...
    all_results.append({'target': target, 'model': 'Mean', 'features': 'vggish', 'rmse': rmse_mean, 'mae': mae_mean, 'r2': r2_mean})
    ...
    all_results.append({'target': target, 'model': 'Linear', 'features': 'vggish', 'rmse': rmse_lr, 'mae': mae_lr, 'r2': r2_lr})
    joblib.dump(lr, models_dir / f'linear_vggish_{target}.pkl')
    ...
    all_results.append({'target': target, 'model': 'Ridge', 'features': 'vggish', 'rmse': rmse_ridge, 'mae': mae_ridge, 'r2': r2_ridge})
    joblib.dump(ridge, models_dir / f'ridge_vggish_{target}.pkl')
    ...
    all_results.append({'target': target, 'model': 'XGBoost', 'features': 'vggish', 'rmse': rmse_xgb, 'mae': mae_xgb, 'r2': r2_xgb})
    joblib.dump(xgb_model, models_dir / f'xgboost_vggish_{target}.pkl')
...
results_path = results_dir / 'vggish_test' / f'vggish_results_{timestamp}.csv'
