...
Usage:
  python scripts/similarity/compare_knn_combined.py --row_idx 42
  python scripts/similarity/compare_knn_combined.py --row_idx 0 -n 20
"""
...
def main():
    ...
    header = f"{'#':<3} | {'OLD COMBINED (Equal Unweighted Split)':<80} | {'Sim':<5} | {'NEW COMBINED (73% Neural / 27% Context)':<80} | {'Sim':<5}"
    print(header)
    print("-" * 125)
    ...
    for r in range(top_n):
        ...
        print(f"{r:<3} | {truncate(text_o, 80):<80} | {old_sims[r]:>6.3f} | {truncate(text_n, 80):<80} | {new_sims[r]:>6.3f}")
