...
|---|---|---|
| **Acoustic kNN Graph** | `knn_audio_top250.parquet` | Pre-computed Top-250 nearest neighbors via cosine indexing on optimal fused CLAP + MERT-330M + VGGish embeddings (1664-D). |
| **Lyric kNN Graph** | `knn_lyric_top250.parquet` | Pre-computed Top-250 nearest neighbors via cosine indexing on optimal fused Harrier-0.6B + Multilingual-E5 embeddings (2048-D). |
| **Mood & Context kNN Graph**| `knn_mood_top250.parquet` | Pre-computed Top-250 nearest neighbors on Unified Mood & Context: Genre Hybrid (40%) + Spotify Audio (30%) + Temporal (15%) + Vocal DSP (15%) (83-D). |
| **Master Multimodal Graph** | `knn_combined_top250.parquet` | Pre-computed Top-250 nearest neighbors on 73% Neural (Audio 38% + Lyric 35%) + 27% Context (Genre 11% + Spotify 8% + Temporal 4% + Vocal 4%) (3795-D). |
| **2D UMAP Projections** | `umap_2d_*.parquet` | 2D projection coordinates for Audio, Lyric, Mood (83-D), and Combined Multimodal representations. *(Qualitative visual projections for exploratory mapping).* |
...
knn_audio = pd.read_parquet('similarity/knn_audio_top250.parquet')
knn_lyric = pd.read_parquet('similarity/knn_lyric_top250.parquet')
knn_mood  = pd.read_parquet('similarity/knn_mood_top250.parquet')
